...
def load_features_and_metadata(
    ...
    **kwargs,
    ...
    tile_indices_list = []
    wsi_paths_list = []
    load_coordinates = "coordinates_path" in df.columns
    coordinates_paths_list = []
    metadata = defaultdict(list)
    metadata_cols = ["case_id"] + [
        x
        for x in df.columns
        if x not in ["wsi_path", "feature_path", "coordinates_path"]
    ]
    ...
    for row in iterator:
        ...
        sampled_indices = torch.randperm(len(_feature), generator=g, device=device)[
            :num_tiles_per_wsi
        ]
    ...
    if load_coordinates:
        res.update({"coordinates_paths": coordinates_paths_list})
    return res
